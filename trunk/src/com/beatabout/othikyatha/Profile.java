...
public class Profile  {
	/**
	 * I have to duplicate the string declarations as the STUPID ANDROID
	 * do not provide any API to get the string resources without passing
	 * around the Context.
	 */
	...
	private static final String NUM_LOCATIONS_PREF = "numLocPref";
	private static final String LATITUDE_PREF = "latitudePref";
	private static final String LONGITUDE_PREF = "longitudePref";
	...
	Collection<Location> getLocations() {
		...
		int size = sharedPreferences.getInt(NUM_LOCATIONS_PREF, 0);
		for (int i = 0; i < size; ++i) {
			Location location = new Location("");
			float latitude = sharedPreferences.getFloat(LATITUDE_PREF + i, 0.0f);
			float longitude = sharedPreferences.getFloat(LONGITUDE_PREF + i, 0.0f);
	...
	void setLocations(Collection<Location> locations) {
		...
		editor.putInt(NUM_LOCATIONS_PREF, locations.size());
		
		int i = 0;
		for (Location location : locations) {
			editor.putFloat(LATITUDE_PREF + i, (float) location.getLatitude());
			editor.putFloat(LONGITUDE_PREF + i, (float) location.getLongitude());
			i++;
		}
		editor.commit();
	}
